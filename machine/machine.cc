...
//	occured (such as the address translation failed). TLB miss?
//
...
//  ASSERT(interrupt->getStatus() == UserMode);
    registers[BadVAddrReg] = badVAddr;  // pagefault的虚拟地址 39
    DelayedLoad(0, 0);			// finish anything in progress
    interrupt->setStatus(SystemMode);   // 陷入内核
    ExceptionHandler(which);		// interrupts are enabled at this point
    interrupt->setStatus(UserMode);     // 从内核返回
}
...
// void Machine::ReadMemory(int addr, uint length, char* data){
//     memcpy(data, mainMemory + addr, length);
// }

// void Machine::WriteMemory(int addr, uint length, char* data){
//     memcpy(mainMemory + addr, data, length);
// }